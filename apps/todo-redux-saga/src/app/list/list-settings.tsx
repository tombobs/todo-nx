...
import {ListThemeSelector} from './list-theme-selector';
    ...
    <List>
      ...
      <Collapse in={sortByOpen} timeout="auto" unmountOnExit>
        <List component="div" disablePadding>
          {sortByOptions.map(s =>
            <ListItemButton key={s.key} sx={{pl: 4, background: s.key === currentSortBy ? (theme.palette.success.main + ' !important') : 'inherit'}} onClick={() => dispatch(sortBy(s.key))}>
      ...
      <ListItemButton onClick={confirmDelete} color='error' >
        ...
        <ListItemText primary='Delete list' color='inherit' primaryTypographyProps={{fontSize: 14, fontWeight: 'medium', color: 'error'}}/>
      </ListItemButton>
    </List>
  );
}
