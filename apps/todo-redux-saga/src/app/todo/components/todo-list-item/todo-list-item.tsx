  ...
  return (
    <Paper>
      <ListItem
        ...
        onClick={onClick}
        sx={{
          marginTop: isFirst ? '0 !important' : 'initial',
          marginBottom: isLast ? '0  !important' : 'initial',
        }}
        className={`
                  ${styles.item} ${item?.isComplete ? styles.complete : ''} 
                  ${opaqueComplete ? styles.opaqueComplete : ''} 
        ...
        <ListItemButton>
          <ListItemIcon>
            <Checkbox
              ...
              onChange={(e) => onToggleComplete(e, item!)}
              ref={checkboxRef}
            />
          </ListItemIcon>

          <ListItemText
            primaryTypographyProps={{ style: {fontWeight: isActive ? 'bold' : 'initial'} }}
            primary={item?.title}
          />
        </ListItemButton>
      </ListItem>
